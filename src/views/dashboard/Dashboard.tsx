...
type ProgressGroup1Item = {
  title: string
  value1: number
  value2: number
}

type ProgressGroup2Item = {
  title: string
  icon: any // CoreUI icons don't export a strict TS type, `any` is fine for CIcon
  value: number
}

type ProgressGroup3Item = {
  title: string
  icon: any
  percent: number
  value: string
...
type TableRowItem = {
  avatar: { src: string; status: 'primary' | 'secondary' | 'success' | 'danger' | 'warning' }
  user: { name: string; new: boolean; registered: string }
  country: { name: string; flag: any }
  usage: { value: number; period: string; color: ProgressColor }
...
const Dashboard: React.FC = () => {
  ...
  const progressGroupExample1: ProgressGroup1Item[] = [
  ...
  const progressGroupExample2: ProgressGroup2Item[] = [
    { title: 'Male', icon: cilUser, value: 53 },
    { title: 'Female', icon: cilUserFemale, value: 43 },
  ]

  const progressGroupExample3: ProgressGroup3Item[] = [
  ...
  const tableExample: TableRowItem[] = [
    {
